...
import { stripCodeFences, ensureCjkFontStack, stripClipAttrs } from "../../util/clean-output";
  ...
  while (!check.ok && repairRounds < 2) {
    ...
    for (const [file, list] of byFile) {
      ...
      const { content: fixed } = await ctx.llm.chat({
        ...
        temperature: 0.3,
        seed: 55,
        // 修复同样要求严格遵守 composition 契约：强制思考 + 低档（与 step4 同理）
        thinking: "enabled",
        reasoningEffort: "low",
      });
      writeFileSync(abs, stripClipAttrs(ensureCjkFontStack(stripCodeFences(fixed))));
    }
