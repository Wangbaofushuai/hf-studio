...
- Produces:
  ...
  - `export interface LlmProvider { id: string; baseURL: string; apiKey: string; models: string[]; temperature?: number }`
...
- Produces:
  - `export interface Services { llm: LlmGateway; judge: Judge; render: (projectDir: string) => RenderService; tts: TtsService }`
...
export interface Services { llm: LlmGateway; judge: Judge; render: RenderService; tts: TtsService }
...
export class PipelineEngine {
  ...
  private async runJob(jobId: string): Promise<void> {
    ...
    for (let step = startStep; step < this.opts.steps.length; step++) {
      ...
      const out = await this.runStepWithRetries(step as StepId, stepFn, jobId, projectDir, job.config.models, prev);
  ...
  private async runStepWithRetries(
    ...
    for (;;) {
      ...
      const ctx: StepContext = {
        ...
        llm: this.opts.services.llm, judge: this.opts.services.judge,
...
- 校验门（gate_failed）：beats 数 3–8；`sum(durationSec)` 与 `config.durationSec` 偏差 ≤ 20%（voiceover=false 时）；voiceover=true 时每个 beat 的 narration 非空
...
- Produces:
  - `export function buildBeatBoundaries(wordsPerBeat: { words: { text: string; start: number; end: number }[] }[], beats: { durationSec: number }[]): { index: number; startSec: number; endSec: number }[]`（纯函数：拼接各 beat 音频，边界 = 累计时长；无配音时用 beats 的 durationSec 归一化到 config.durationSec）
...
import type { TtsService } from "../tts/service";
import type { StepId, JobConfig } from "../types";
...
export function createServer(opts: {
  ...
  app.post("/api/jobs", async (c) => {
    ...
    if (![5, 120].every((_, i) => i === 0) || durationSec < 5 || durationSec > 120) {
    ...
    opts.store.updateJob(jobId, { config: { idea, durationSec, format, voiceover, voice, language, models: { default: model }, materials: { images, audio } } });
...
export default function ModelSelect({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-md border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm">
      {!models?.providers.length && <option value="">（未配置模型，请在 server/config.json 配置）</option>}
      {models?.providers.map((p) => p.models.map((m) => (
...
要求用户把 `server/config.json` 填好（DeepSeek/GLM 等任一 OpenAI 兼容 provider + apiKey，judge 可复用同一 provider 或另配）。
...
3. **类型一致性**：`StepContext`/`StepOutput`/`StepResult`/`Boundary`/`Beat` 跨任务签名一致；`render` 在引擎与步骤间为工厂签名（Task 13 注记已统一为 `(projectDir) => RenderService`），步骤内 `ctx.render.lint()` 等调用保持不变。
