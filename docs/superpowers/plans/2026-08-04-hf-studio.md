...
> 注：`deepseek-v4-flash` 为默认模型（用户指定）；若实际渠道的模型名/端点不同，用前端自定义渠道或改 config.json。
...
要求用户把 `server/config.json` 填好（OpenAI 兼容 provider + apiKey，默认模型 `deepseek/deepseek-v4-flash`）；**也可以不改文件**——把渠道信息填进前端「自定义模型渠道」区，通过 Web 界面建任务跑通全流程（等效验收，推荐）。
